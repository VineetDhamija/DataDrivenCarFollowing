...
'''
Picked from : 
https://github.com/Horacehxw/traffic_simulation/blob/master/src/IDM.py
...
class IDM():
    ...
    def initialize(self):
        ...
        for s in range(1, self.ismax+1):
            ...
            for k in range(0, kmax):
                ...
                acc = self.a * \
                    (1.-np.power(Ve/self.v0, self.delta) - (s_star**2) / (s**2))
                Ve += acc * dt
    ...
    def calc_acc(self, bwd: Moveable, fwd: Moveable):
        ...
        acc = self.a * (1 - np.power(vel / self.v0,
                        self.delta) - (s_star ** 2) / (s**2))
